...
- **로그인 및 회원가입**  
  클라이언트에서 카카오 로그인을 하고 나서 회원번호와 함께 API를 호출하면 서버에서 해당 기능에 맞는 서비스를 제공합니다.  
  카카오 회원번호는 각 유저마다 유일하기 때문에 토큰의 의미로 사용했습니다.   
  회원가입은 카카오 회원번호와 유저의 기본정보를 저장하는 방식으로 진행됩니다.  

- **유저 데이터**
  
  <유저 데이터 존재 유무 반환>  
  클라이언트에서 회원가입 유무에 따라 메인화면 또는 회원가입 화면으로 이동할 때 사용됩니다.  
  
  <유저 데이터 반환 및 수정>    
  클라이언트에게 필요한 유저의 기본 정보와 현재 잔고 데이터를 제공합니다.    

- **신발 장식**
  
  <신발 장식 데이터 조회 및 수정>  
  신발에 부착된 신발 장식 데이터 및 유저가 소유한 신발 장식 데이터를 조회 및 수정할 수 있습니다.  
  유저가 신발 장식을 구매한 경우에는 유저의 잔고를 줄이고 구매한 신발 장식 개수를 증가시킵니다.  

- **운동 데이터**
  
  <운동 기록 수정 및 조회>  
  유저의 운동 기록을 저장하고 (총 걸음수 / 1000)만큼의 재화를 지급합니다.  
  운동 기록 조회는 검색하고 싶은 기간의 날짜를 입력받고 운동 데이터의 Id 리스트를 반환합니다.
  
  <걸음수 조회>  
  클라이언트에게 주, 월, 연 단위로 유저의 총 걸음수를 반환합니다.  
  해당 데이터는 유저가 각 기간별 걸음수를 그래프의 형태로 확인할 수 있도록 도와줍니다.    
